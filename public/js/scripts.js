...
function backgroundImg_init() {
  var pageSection = $(
    '.home-section, .page-section, .small-section, .blog-section, .small-section-2, .breadcrumb-section'
  );
...
function top_panel_search_init() {
  $('.main-nav .search-menu').on('click', function () {
    ...
    $(this)
      .parents('.main-nav')
      .find('.search-menu-cont .search-field')
      .focus();
  });
  ...
  $(window).scroll(function () {
    if ($('.sticky-header').hasClass('top-fixed')) {
      if ($(window).scrollTop() > 10) {
        ...
        $(
          '.main-nav, .nav-logo-wrap .logo, .mobile-nav, .search-back-button'
...
function checkout_open_field() {
  $(
    '.payment_method_cheque input, .payment_method_paypal input, .payment_method_bank input'
  ).on('click', function () {
    ...
    if ($(x).hasClass('active')) {
      ...
      $(
        '.payment_method_cheque .payment_box, .payment_method_paypal .payment_box, .payment_method_bank .payment_box'
      )
        .removeClass('active')
        .slideUp();
      ...
      map: {
        options: {
          ...
          styles: [
            ...
            {
              featureType: 'transit',
              stylers: [{ color: '#575341' }, { visibility: 'off' }],
            },

            {
              featureType: 'road.highway',
              elementType: 'geometry.stroke',
              stylers: [{ visibility: 'on' }, { color: '#e6e5de' }],
            },
...

            {
              featureType: 'poi',
              elementType: 'geometry.fill',
              stylers: [{ visibility: 'on' }, { color: '#b0bfd1' }],
            },
...

            {
              ...
              elementType: 'geometry.fill',
              stylers: [{ visibility: 'on' }, { color: '#f8f8f6' }],
            },
...

            {
              featureType: 'administrative',
              elementType: 'labels.text.fill',
              stylers: [{ visibility: 'on' }, { color: '#575341' }],
            },
...
function shop_price_slider() {
  ...
  if (shop_price_slider_params.min_price)
    current_min_price = parseInt(shop_price_slider_params.min_price, 10);
  if (shop_price_slider_params.max_price)
    current_max_price = parseInt(shop_price_slider_params.max_price, 10);
  $('body').bind(
    'price_slider_create price_slider_slide',
    function (event, min, max) {
      if (shop_price_slider_params.currency_pos === 'left') {
        $('.price_slider_amount span.from').html(
          shop_price_slider_params.currency_symbol + min
        );
        $('.price_slider_amount span.to').html(
          shop_price_slider_params.currency_symbol + max
        );
        ...
        $('.price_slider_amount span.from').html(
          min + shop_price_slider_params.currency_symbol
        );
        $('.price_slider_amount span.to').html(
          max + shop_price_slider_params.currency_symbol
        );
